...
// ==/UserScript==
// © 2025 Gheat.net. This script is licensed under CC BY-NC-ND 4.0
// https://creativecommons.org/licenses/by-nc-nd/4.0/
// You may use and share this script, but not modify or redistribute it without permission, please send contribution ideas to the discord.

/* --------------------------------------- */
// ! YOU ARE CURRENTLY ON A DEVELOPER BETA !
/* --------------------------------------- */
// this WILL have errors, and bugs. please dont
// post bugs from this version in complaints as
// bugs from this version should be sent in the
// DEV-BETA issues forum!

/*  TODO  */
/* This is the To-Do updated as of November 22nd 2025
1. i want to move all of the local checks to its own server and move the gheat.net
to a server aswell and then have this in the same PHP server, so that you only have
to use one server for both the gheat.net/shredsauce/ and the script, this would help
a ton with local checks and will have better load times and will have the ability to
be updated real time and have functions that can be adaptive unlike the static github
page, it will also fix the problem with the server checker being unable to actually
test the functionallity of the website especially the shredsauce.com not shredsauce.com
/beta, there are alot of other things it would make simpler but i wont explain them all.

1. a) a sign in page would be pretty good for saving themes and other stuff that i could
save like SLVSH scoring etc etc, this could also form into a leader board for slvsh but
that brings up the cheating question because the script allows for players to just spam
the slvsh scoring keys, making this unable to work, i could fix this via a time based
thing like its not possible to score a letter ever 0.5 seconds.

1. b) make the server updater work via the server aswell would be helpful and a
integraty script to check the integrity of the script after its shared for leaderboard
reasons, the server updater could also update the script automaticly when it sees that
it is out of date, also im not sure if the server updater works well anymore with the
way it is programmed currently, it should be switched off of greasy fork as i dont
like the greasy fork updater, it doesnt work at all and doesnt notify, this will be
fixed with the extension making it wayyy easier to update the script automatically
and will make small updates alot more easy to push and not bother the end-user when
the script needs to be updated.

1. c) Language options, currently this is only supported in english and that is not 
very helpfull as i have gotten feedback from users that speak other languages other
than english that there translater extensions cant translate the script as it is
overlayed rather than being baked into the website, currently languages that are
being requested include but do not extend to; German, Russian, Spanish, Swedish. 
this could link back to the server/ login, saving this data there but could be
done right now via the localchecks. i intend to make translations via AI as i do 
not have the budget to pay people to translate but, if you want to translate the 
script, please DM me via Discord!

1. d) menu redesign i want it to look more like its supposed to be there than how it
is currently looking very out of place and different from the rest of the website, i
was thinking this could be done by using the space underneath the player window, i
also want to center the game player window and center the text boxes that follow it
this could also just be done by fully reorginizing the website or hosting the webite
via my servers but this might not work as shredsauce has blockers to stop the use of
the game on websites other than shredsauce.com but im sure this can be bypassed, jst
is out of my knowledge and if anyone has experience in doing this please reach out to
me!

2. SLVSH updates, this includes being able to write down the trick that you will do
this could lead back into 1a with the leaderboard not being spamable and make the text
box have some form of checker to check that your not just writing a or something to
bypass this, a bug that will need to also be fixed is the button focusing in shredsauce
as it is very annoying if you do not realize,

3. This is arguabley the longer of the To-Do's as it as at the end but this is covering
the extension, this will not be done for quite a while and a developer alpha should come
in the months following december 2025 but do not rely on this as a accurate reperesentaion
of time, as i am very busy at the moment with the comp-sci team i am running and school,

3. a) i want this to mainly be hosted on a server not on the users local machine as it
could be manipulated very easily, in the extension i want to add a function for a global
chat that is filtered, and could have people and there status of what server they are on
/ what map they are on currently. i am not currently to sure of how i will acomplish this
but it is very possible. another addition to this is the leaderboard and ranks, and this
could link to the shredsauce discord, im not sure how this would be formated and this cld
aline with 1d very well as i would need to re organize the website as we know it currently
i want this to be like a whole new thing thats why this will be V3 of the script being the
3rd major change since the release. This could be linked via discord bot maybe make this 
like a status based thing that shows what map and rank etc i think that some of the 

Thank You! for using my script and helping contribute, this wouldnt be possible with out
you guys.
*/
  ...
  `;
    ...
    window.addEventListener('load', () => {
      ...
      const allowedIds = [
          ...
          'butta-edit', 'butta-video', 'fakedocs'
      ];
...
favicon.href = 'https://raw.githubusercontent.com/Gheat1/Gheat.net/refs/heads/main/favicon.png';
document.head.appendChild(favicon);

// Fake Google Docs Toggle
const fakeDocs = document.createElement('div');
fakeDocs.id = 'fakedocs';
Object.assign(fakeDocs.style, {
  position: 'fixed',
  top: '0',
  left: '0',
  width: '100vw',
...
// you could edit this to change what it says maybe for future could asign like a custom doc loader to auto load it in
fakeDocs.innerHTML = `
  <div style="background: white; padding: 10px 20px; border-bottom: 1px solid #ddd; display: flex; align-items: center;">
    <div style="font-size: 16px; font-weight: bold; margin-right: 20px;">Untitled document</div>
    <div style="font-size: 14px; color: #5f6368;">File &nbsp; Edit &nbsp; View &nbsp; Insert &nbsp; Format &nbsp; Tools &nbsp; Extensions &nbsp; Help</div>
  </div>
  <div style="display: flex; justify-content: center; margin-top: 30px;">
    <div style="background: white; width: 816px; min-height: 1056px; box-shadow: 0 0 8px rgba(0,0,0,0.2); padding: 60px; font-size: 14px; line-height: 1.6;" contenteditable="true">
      Start typing your document here...
    </div>
  </div>
`;

document.body.appendChild(fakeDocs);
document.title = 'Docs - Untitled Document';
let docsVisible = false;
window.addEventListener('keydown', (e) => {
  if (e.key.toLowerCase() === '=' && !e.repeat) {
    docsVisible = !docsVisible;
    fakeDocs.style.display = docsVisible ? 'block' : 'none';
    document.title = docsVisible ? 'Docs - Untitled Document' : 'Shredsauce';
  }
});
  ...
  `;
    ...
    /********** Tools / SLVSH stuff **********/
    /* currently there is a glitch with the slvsh menu that sometimes */
    /* after using the score board it still thinks you are inside the */
    /* the menu ruining scoring etc this could be fixed by changing   */
    /* it grabs the cursor etc but this glitch is not important tdl8r */
    ...
    // this could be changed to show the new extension to change the textures 
    // reference top todo
...
`;
// the settings HUD has a major bug that broke the website on
// 6/10 machines i have tested it on, this bug is from when
// mouse clicks on the settings hud or trys to grab the top to
// move it around i have no clue how to fix this, and ai cant
// either, i think the problem stems from this being written in
// javascript via tampermonkey and the bug is being caused by
// that, this should be fixed in the V3 if i can figure it out
// by then when i transform this to a extension instead of a 
// tampermoneky script there will be updates in my discord,
// showing all of the open problems and whats planed for V3
        ...
        // currently this function will sometimes not save information
        // especially if you have 2 seperate google profiles this in the
        // future could be optomized via using my server to host the 
        // check data, although this will be complicated and might be
        // available in the V3 via extension, and that is in progress
        // currently with the main problem being google, mozilla is
        // working faster and a license is being aquired - Nov 22 Gheat
...
// this will be moved to a server aswell
        ...
        // in theory you could add your own buttons here for different color themes you want
        // but i think that just editing this to your main and once i add server functions i
        // can just make a sign in type thing that could save this in there so you could have
        // your own presets. if anyone can help with server managment it would be very helpfull
        // and should speed up the process by quite alot, at the moment i have a cluster of
        // prodesks that are running docker, and i was playing with PHP but it is alot more
        // dificult then i thought this will probably come around when i move the gheat.net over
        // to my servers, thank you for your patience with the servers. this script will probably
        // be moved over and the script that will be on your end will be alot more simple just
        // a call to my server to accsess the script, but this could also be loaded manually
        // this project will be kept open source via the format of it being through a script or
        // extension.
...
// this is another failed atempt at gettign the volume to work but to still no avail
// if someone knows please tell me how, i do not understand!
    ...
    // Sausce Server Status

    // this function is often broken now aday at the fault of
    // the game as the shredsauce.com will not allow you to click
    // play but the shredsauce.com/beta will this will need to be
    // fixed somehow but im not sure yet as it is hard to hook
    // the version via the discord bot has more accurate results
    // please check the shredsauce status bot the github repo is
    // linked in my portfolio, as of November 22nd this function
    // is very likely to go extinct as shredsauce as fixed the 
    // server memory leak and we have not experienced a problem
    // that removes game function since, the main problem is the
    // beta version being the only version still working after
    // the memory leak fix so if you experience problems it is
    // due to not being on the shredsauce.com/beta
    ...
    // Site Volume
    // currently this function is broken i have no clue how to 
    // fix it, this seams to be a browser by browser error
    // with failing to have a constant volume header to grab
    // maybe this could be fixed via hooking game header to
    // limit volume but i would have to dig to find the header
    // this should be fixed on v2.1.1
    ...
    // Update Notifier - COMMENTED OUT
    /*
    // this current version is out of date as this is a dev beta
    // this code will not function in dev beta's as i will not be 
    // posting to greasy for only github, as said in the repo 
    // developer betas are very unstable and at times can ruin
    // frames and brick the game on some devices/ browsers
    // v2.1.0 developer beta is still being worked on actively
    // although progress has slowed down a lot from school &
    // other things. sorry for the delays and getting this update
    // out
